...
class RandomAgent:
    def __init__(self):
        #self.random = random.Random(np.random)
        pass
...
class MinimaxAgent:
    ...
    def get_move(self, board):
        ...
        rand = np.random.choice(best_moves)
        return rand
...
class KalahaFight: #(KalahaBoard):
    ...
    def fight(self):
        ...
        for Key,[Depth,Specs] in playingdict.items():
            temp_score_array = []
            board = KalahaBoard(self.number_of_cups, self.stones)
            agent1 = MinimaxAgent(6,alpha_beta_pruning=True)
            agent2 = MinimaxAgent2(2,alpha_beta_pruning=True)
            #agent2 = RandomAgent()
            ...
            for i in range(10):
                ...
                if invalid_count > 2:
                    if last_invalid_player == 0:
                        temp_score_array.append(1)
                        # wins_agent1 += 1
                        print("player 1 wins")
                    else:
                        # wins_agent2 += 1
                        temp_score_array.append(2)
                        print("player 2 wins")
                else:
                ...
                board.reset_board()
            temp_dict = {f'{Key}': temp_score_array}
            ...
            plt.plot(y,x_mean,label=Key)
            plt.legend()

        # for Key_score, results in score.items():
        #     win1 = sum(results[results==1])
        #     win2 = sum(results[results==2])
        #     win3 = sum(results[results==2])

        #     print(f'{Key_score} had win rate {win1/10}, with {win3} draws\n')
